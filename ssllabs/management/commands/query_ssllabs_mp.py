    ...
    for data in finished:
        ...
        for ep in data.get('endpoints'):
            ...
            if ep.get('statusMessage').upper() == 'READY' and ep.get('progress') == 100:
                ...
                startTime = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(data.get('startTime') / 1000))
                endTime = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(data.get('testTime') / 1000))
                ...
                h.endTime = endTime
                h.notBefore = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(cert.get('notBefore') / 1000))
                h.notAfter = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(cert.get('notAfter') / 1000))
                h.status = data.get('status')
                h.statusMessage = data.get('statusMessage')
                h.save()
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #Update status of these records to RUNNING
